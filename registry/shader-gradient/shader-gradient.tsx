...
function compile(gl: WebGLRenderingContext, type: number, source: string) {
  ...
  if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
    // Without this the canvas just stays blank behind the CSS fallback
...
export function ShaderGradient({
  ...
  // Tracks shadcn theme for the CSS fallback (WebGL reads darkRef each frame)
  ...
  propsRef.current = { colors, speed, blur, intensity, interactive, theme }
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      const { colors, speed, blur, intensity, interactive, theme } =
        propsRef.current
      const time = reduceRef.current ? frozenTime : (now - start) / 1000
      const dark = darkRef.current
      const paletteSrc = colors ?? (dark ? DARK_COLORS : LIGHT_COLORS)
      ...
      gl.uniform1f(uSpeed, reduceRef.current ? 0 : speed)
      gl.uniform1f(uBlur, Math.min(1, Math.max(0, blur)))
      ...
      gl.uniform2f(uMouse, interactive ? m.x : 0.5, interactive ? m.y : 0.5)
  ...
  return (
    <div
      ...
      style={{
        // CSS fallback if WebGL is unavailable (inline — Tailwind can't parse multi-layer arbitrary gradients)
